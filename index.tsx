...
import { 
  ...
  DollarSign, TrendingUp, CreditCard, Briefcase
} from 'lucide-react';
import { 
...
const EXPENSE_CATEGORIES = [
  ...
  { name: 'Rent/Housing', color: '#60A5FA' },
...
const App: React.FC = () => {
  ...
  // Load data
  useEffect(() => {
    const saved = localStorage.getItem('dordam_data_v2');
    if (saved) {
      try {
  ...
  // Save data
  useEffect(() => {
    localStorage.setItem('dordam_data_v2', JSON.stringify({ transactions, debts, profile }));
    ...
    <div className="min-h-screen text-slate-200">
      <header className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-xl border-b border-slate-800 px-6 py-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white"><Wallet size={22}/></div>
          <div><h1 className="text-xl font-black text-white">Dor-Dam</h1><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Expense Ledger</p></div>
        </div>
        <button onClick={() => setIsProfileModalOpen(true)} className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-800 hover:bg-slate-900 transition-all">
          <User size={14} className="text-indigo-400"/><span className="text-xs font-bold">{profile.name}</span>
        </button>
      </header>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {[
                ...
                <div key={i} onClick={() => setActiveTab(item.target as any)} className="glass-card p-5 rounded-3xl flex flex-col justify-between hover:scale-[1.02] transition-all cursor-pointer group">
                  <div className={`w-10 h-10 bg-${item.color}-900/30 text-${item.color}-400 rounded-xl flex items-center justify-center mb-4`}><item.icon size={20}/></div>
                  <h3 className="text-xl md:text-2xl font-black text-white">৳{item.value.toLocaleString()}</h3>
                  <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">{item.label}</p>
                </div>
              ))}
            </div>
            ...
            <div className="glass-card p-8 rounded-[2rem]">
               ...
               <div className="space-y-4">
                 {recentActivity.length === 0 ? (
                   <div className="py-10 text-center text-slate-600 text-xs font-bold uppercase">No records found</div>
                 ) : (
                   recentActivity.map(act => (
                     <div key={act.id} className="flex items-center justify-between p-3 hover:bg-white/5 rounded-2xl transition-all">
                       <div className="flex items-center gap-3">
                         <div className={`p-2 rounded-lg ${act.type === 'income' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>{act.type === 'income' ? <ArrowDownLeft size={16}/> : <ArrowUpRight size={16}/>}</div>
                         <div>
                           <p className="text-sm font-bold text-white">{act.category}</p>
                           <p className="text-[10px] text-slate-500 font-bold uppercase">{act.note || 'General'}</p>
                         </div>
                       </div>
                       <span className={`text-sm font-black ${act.type === 'income' ? 'text-emerald-400' : 'text-rose-400'}`}>{act.type === 'income' ? '+' : '-'}৳{act.amount.toLocaleString()}</span>
                     </div>
                   ))
                 )}
               </div>
          ...
          <div className="space-y-6 animate-entry">
            ...
            <div className="glass-card rounded-[2rem] overflow-hidden">
              {transactions.length === 0 ? (
                <div className="p-20 text-center text-slate-500 font-bold uppercase text-xs">No entries yet.</div>
              ) : (
                transactions.map(t => (
                  <div key={t.id} className="p-6 flex items-center justify-between border-b border-slate-800/50 hover:bg-white/[0.02] group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-bold uppercase">{t.date} • {t.note || 'No note'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      ...
                      <button onClick={() => setTransactions(transactions.filter(x => x.id !== t.id))} className="text-slate-600 hover:text-rose-500 transition-colors opacity-0 group-hover:opacity-100 p-2">
                        <Trash2 size={16}/>
                      </button>
                    </div>
                  </div>
        ...
        {activeTab === 'debts' && (
          <div className="space-y-6 animate-entry">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {debts.length === 0 ? (
                <div className="col-span-2 glass-card p-20 text-center text-slate-500 font-bold uppercase text-xs">No debts tracked.</div>
              ) : (
                debts.map(d => (
                  <div key={d.id} className={`glass-card p-6 rounded-[2rem] border-l-4 ${d.status === 'repaid' ? 'border-emerald-500' : 'border-amber-500'}`}>
                    <div className="flex justify-between mb-4">
                      <div>
                        <h4 className="text-lg font-black text-white">{d.person}</h4>
                        <p className="text-[10px] text-slate-500 font-bold">{d.date}</p>
                      </div>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => setDebts(debts.map(x => x.id === d.id ? {...x, status: x.status === 'repaid' ? 'pending' : 'repaid'} : x))} className="flex-1 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all">
                        {d.status === 'repaid' ? 'RE-OPEN' : 'SETTLE'}
                      </button>
                      <button onClick={() => setDebts(debts.filter(x => x.id !== d.id))} className="p-2 text-slate-600 hover:text-rose-500">
                        <Trash2 size={16}/>
                      </button>
                    </div>
                  </div>
      ...
      <button onClick={() => setIsAddModalOpen(true)} className="fixed bottom-24 right-8 w-14 h-14 bg-indigo-600 text-white rounded-2xl shadow-xl shadow-indigo-600/30 flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-40">
        <PlusCircle size={28}/>
      </button>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-slate-950/90 backdrop-blur-xl border-t border-slate-800 p-4 flex justify-around md:hidden z-50">
        {[
          ...
          <button key={t.id} onClick={() => setActiveTab(t.id as any)} className={`flex flex-col items-center gap-1 transition-all ${activeTab === t.id ? 'text-indigo-500 scale-110' : 'text-slate-600'}`}>
            <t.icon size={20}/>
      ...
      <div className="hidden md:flex fixed left-8 top-1/2 -translate-y-1/2 flex-col gap-4 bg-slate-900/50 backdrop-blur-xl p-3 rounded-3xl border border-slate-800 shadow-2xl z-50">
         {[
          ...
          <button key={t.id} onClick={() => setActiveTab(t.id as any)} className={`p-4 rounded-2xl transition-all ${activeTab === t.id ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-800'}`} title={t.label}>
            <t.icon size={24}/>
          </button>
        ))}
      </div>
        ...
        <form className="space-y-4" onSubmit={(e: any) => { 
          ...
          <div className="flex bg-slate-800 p-1 rounded-xl">
            <label className="flex-1">
              ...
              <div className="text-center py-2 rounded-lg cursor-pointer peer-checked:bg-slate-700 peer-checked:text-rose-400 text-slate-500 font-bold text-xs">Expense</div>
            </label>
            <label className="flex-1">
              <input type="radio" name="type" value="income" className="hidden peer"/>
              <div className="text-center py-2 rounded-lg cursor-pointer peer-checked:bg-slate-700 peer-checked:text-emerald-400 text-slate-500 font-bold text-xs">Income</div>
            </label>
          </div>
          <input name="amount" type="number" placeholder="Amount (৳)" required className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl text-white font-bold text-lg"/>
          <select name="category" className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl text-white font-bold">
            {EXPENSE_CATEGORIES.map(c => <option key={c.name} value={c.name}>{c.name}</option>)}
          </select>
          <input name="date" type="date" defaultValue={new Date().toISOString().split('T')[0]} className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl text-white"/>
          <input name="note" placeholder="Notes..." className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl text-white"/>
          <button className="w-full py-4 bg-indigo-600 text-white font-black rounded-xl shadow-lg">Save Entry</button>
        </form>
      </Modal>
      ...
      <Modal isOpen={isDharModalOpen} onClose={() => setIsDharModalOpen(false)} title="Track New Dhar">
        <form className="space-y-4" onSubmit={(e: any) => { 
          ...
          <input name="person" placeholder="Person Name" required className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl text-white font-bold"/>
          <input name="amount" type="number" placeholder="Amount (৳)" required className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl text-white font-bold text-lg"/>
          <input name="date" type="date" defaultValue={new Date().toISOString().split('T')[0]} className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl text-white"/>
          <textarea name="note" placeholder="Reason..." className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl text-white h-24"/>
          <button className="w-full py-4 bg-amber-500 text-slate-950 font-black rounded-xl">Save Dhar</button>
        </form>
      </Modal>
      ...
      <Modal isOpen={isProfileModalOpen} onClose={() => setIsProfileModalOpen(false)} title="Settings">
        <form className="space-y-4" onSubmit={(e: any) => { 
          ...
          <label className="block text-xs font-black text-slate-500 uppercase tracking-widest">Display Name</label>
          <input name="name" placeholder="Your Name" defaultValue={profile.name} required className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl text-white font-bold"/>
          <label className="block text-xs font-black text-slate-500 uppercase tracking-widest">Monthly Limit</label>
          <input name="budget" type="number" placeholder="Budget Limit" defaultValue={profile.monthlyBudget} required className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl text-white font-bold"/>
          <button className="w-full py-4 bg-indigo-600 text-white font-black rounded-xl">Save Settings</button>
        </form>
      </Modal>
    </div>
  );
